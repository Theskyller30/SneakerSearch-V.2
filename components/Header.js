...
import { calculateTotal } from '../store/actions/cart'
...
export default function Header() {
  ...
  return (
    <header className="header">
          ...
          <div className="header__logo">
            <Link href="/">
              <a className="header__logo-link">
                <img className="header__logo-img" src="/assets/images/logo.png" alt="SneakerSearch" />
              </a>
            </Link>
            <div className="header__logo-content">
              <span className="header__logo-text">SneakerSearch</span>
              <span className="header__logo-tagline">Compra los mejores sneakers</span>
            </div>
          </div>
          <ul className="header__menu">
            <li className="header__menu-item">
              <button className="header__menu-cart" onClick={handleCart}>
                ...
                {total !== 0 && <span className="header__menu-cart__price">{convertPrice(total)} Cop.</span>}
              </button>
            </li>
            <li className="header__menu-item">
              <Link href="/favorites">
                <a className={'header__menu-link' + (router.pathname === '/favorites' ? ' header__menu-link--active' : '')}>
            ...
            <li className="header__menu-item">
              <Link href="/profile">
                <a className={'header__menu-link' + (router.pathname === '/profile' ? ' header__menu-link--active' : '')}>
                <svg width="20" height="20" viewBox="0 0 20 20" fill="none" xmlns="http://www.w3.org/2000/svg">
      ...
      <Cart show={showCart} cart={cart} total={total} handleShowCart={handleCart} />
    </header>
  )
}
