...
export default function CartItem({ item }) {
  ...
  return (
    <div className="cart-item">
      <img className="cart-item__img" src={item.imageUrl} alt={item.title} />
      <div className="cart-item__info">
        <span className="cart-item__title">{item.title}</span>
        <span className="cart-item__price">{convertPrice(item.price)} Cop. x {item.quantity}</span>
      </div>
      <button className="cart-item__remove" onClick={() => dispatch(removeItem(item.id))}>
